...
| Concept            | Definition                                                                                                                                                               |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Mesh Workspace** | A workspace with `bridge_all = true`. Membership _is_ the bridge: an agent tab added here joins everyone's roster; closing it removes it.                                |
...
- **Addressing.** The send tool gains a `recipient` and a `topic`. Both are **optional on
  ...
  maiTerm-minted handle**; the human-editable role name is the _display_ label, not the
...
- **Purpose is OPTIONAL** (decision, post-Phase-1). A well-named tab already carries its
  domain, and the agent _self-declares_ its scope into its status note on join ("What I
...
- A visible **per-topic turn count** on the edge makes a runaway ping-pong _visible_ so the
...
8. **Human is a director, not a mesh peer** — the human tells an _agent_ what needs done;
   ...
   control; they work _through_ agents.
