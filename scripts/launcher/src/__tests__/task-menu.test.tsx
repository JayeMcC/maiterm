...
describe('<TaskMenu>', () => {
  it('renders header with clone name + task count', () => {
    const { lastFrame } = render(<TaskMenu cloneName="developing" tasks={SAMPLE_TASKS} onSelect={() => undefined} onQuit={() => undefined} />);
  ...
  it('arrow-down moves the cursor and Enter calls onSelect with the focused task', async () => {
    ...
    const { stdin, lastFrame } = render(<TaskMenu cloneName="developing" tasks={SAMPLE_TASKS} onSelect={onSelect} onQuit={() => undefined} />);
    stdin.write('\x1B[B'); // ↓
  ...
  it('wraps from last back to first on arrow-down', async () => {
    const { stdin, lastFrame } = render(<TaskMenu cloneName="developing" tasks={SAMPLE_TASKS} onSelect={() => undefined} onQuit={() => undefined} />);
    stdin.write('\x1B[B');
    stdin.write('\x1B[B');
    stdin.write('\x1B[B');
    await new Promise((r) => setTimeout(r, 10));
  ...
  it('q triggers onQuit', async () => {
    const onQuit = vi.fn();
    const { stdin } = render(<TaskMenu cloneName="developing" tasks={SAMPLE_TASKS} onSelect={() => undefined} onQuit={onQuit} />);
    stdin.write('q');
    await new Promise((r) => setTimeout(r, 10));
