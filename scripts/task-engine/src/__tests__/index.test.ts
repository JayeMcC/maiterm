...
describe('resolveTask', () => {
  it('returns a tree with all string fields variable-substituted', () => {
    ...
    expect(tree.task.command).toBe('bash /workspace/forwood-one_developing/.vscode/scripts/tasks/dev-api.sh');
  ...
  it('resolves nested dependency trees', () => {
    ...
    expect(tree.dependsOn.map((c) => c.task.label)).toEqual(['Require devcontainer', 'API', 'WEB']);
  });
