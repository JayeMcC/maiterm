 ...
 */
export function buildTaskTree(rootLabel: string, allTasks: readonly Task[], options: ExecutionContextOptions = {}): TaskTreeNode {
  ...
  function buildNode(task: Task): { node: TaskTreeNode; gateInSubtree: boolean } {
    if (path.includes(task.label)) {
      throw new Error(`Dependency cycle: ${[...path, task.label].join(' -> ')}`);
    }
    path.push(task.label);

    const depLabels = task.dependsOn === undefined ? [] : Array.isArray(task.dependsOn) ? task.dependsOn : [task.dependsOn];
    ...
    const executionContext: ExecutionContext = task.label !== gateLabel && gateInSubtree ? 'container' : 'host';
 ...
 */
export function deriveExecutionContext(rootLabel: string, allTasks: readonly Task[], options: ExecutionContextOptions = {}): ExecutionContext {
