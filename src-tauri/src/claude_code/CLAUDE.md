...
_inside_ a subagent with `agent_id`/`agent_type` (same `session_id` as the parent — no
...
**Inferred affinity may act on a tab, never speak as it.** The ambiguity guard above reasons about _counts_, not identity — an MCP tool call carries nothing that identifies its caller, so recovery is structurally a guess and cannot be made correct. `connection_tabs` therefore stores `TabAffinity { tab_id, stated }`: `stated: true` when the agent named its own tab via `initSession`, `false` when a reconnect made us infer it. Tools in `PEER_ADDRESSING_TOOLS` — the bridge/mesh roster and send tools, plus `postCommsReply`/`startCommsThread` — are refused on an inferred binding (`refuse_on_inferred_identity`) with an error telling the agent to run `/maiterm init`. Passing an explicit `tabId` also satisfies it. Everything else keeps the reconnect convenience, since a wrong guess there is a wrong _read_, not this agent's words appearing in a stranger's terminal under that tab's identity. Observed for real: after an app restart a bridged agent's reconnect inferred a tab in an unrelated 16-agent mesh workspace, and `listBridgedPeers` answered for it with no hint the roster wasn't its own; the next step would have been to pick a recipient from that list and send. Recovery decisions log at INFO and refusals at WARN — production runs at INFO, so the previous `log::debug!` lines meant every one of these decisions was invisible in exactly the log you reach for afterwards.
