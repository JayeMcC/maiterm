  ...
  async function refreshLiveness() {
    ...
    for (const pane of ws.panes) {
      for (const tab of pane.tabs) {
        ...
        seen.add(tab.id);
        try {
          const l = await getAgentLiveness(inst.ptyId);
          running[tab.id] = l.agent_running || l.ssh_foreground;
        } catch {
          /* PTY may have just closed — keep the prior reading */
        }
      }
    }
  ...
  const rows = $derived.by((): Row[] => {
    ...
    for (const pane of ws.panes) {
      for (const tab of pane.tabs) {
        ...
        const status: Status = suspended ? 'suspended' : !named ? 'unnamed' : agentLive ? 'ready' : wasAgent ? (running[tab.id] ? 'needs-init' : 'dropped') : 'not-registered';
