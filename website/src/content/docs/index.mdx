...
<CardGrid stagger>
  ...
  <Card title="Mesh a whole workspace" icon="random">
    Bridge connects two agents — a mesh connects them all. Flip a workspace into a mesh and every named agent can message any other by role, addressed and routed so a rename never misroutes. Conversation groups into topics with soft, hard, and time-to-live loop caps so agents can't thrash. Open the cockpit (`Cmd+Shift+M`) for a live conversation graph, a topic board, and a focused stage + filmstrip view of every agent.
  </Card>
